...
(defn basic-actions [{:keys [side active-player end-turn runner-phase-12 corp-phase-12 me]}]
  [:div.panel.blue-shade
   ...
   (when (= side :runner)
     [:div
      (when
        ...
        [cond-button (tr [:game.remove-flatline (render-message "[Click], 1[Credits]:Remove Flatline")])
      ...
      (when
        ...
        [cond-button (tr [:game.remove-stun (render-message "[Click], 2[Credits]: Remove Stun")])
         (and (not (or @runner-phase-12 @corp-phase-12))
              (playable? (get-in @me [:basic-action-card :abilities 8])))
         #(send-command "remove-stun-counter")])
      (when
          (pos? (get-in @me [:doppelganger-counter]))
        [cond-button (tr [:game.remove-doppelganger (render-message "[Click], 4[Credits]: Remove Doppelganger")])
         (and (not (or @runner-phase-12 @corp-phase-12))
   ...
   (when (and (= side :corp) ;;payment for acme S&L
              (playable? (get-in @me [:basic-action-card :abilities 8])))
     [cond-button (tr [:game.pay-acme (render-message "[Click], 12[Credits]: Pay ACME")])
      (and (not (or @runner-phase-12 @corp-phase-12))
           (playable? (get-in @me [:basic-action-card :abilities 8])))
